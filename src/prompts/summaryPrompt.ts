...
const SYSTEM_PROMPTS = {
  long: `You are tasked with writing a comprehensive summary the following text so that readers will have a full understanding of the text, without need of referencing the source material.
...
- ...`,
  ...
  medium: `Your task is to create a concise yet informative summary of the following text, giving readers a clear understanding of the key themes and points without delving into excessive detail.
...
- ...`,
  ...
  short: `Condense the following text into a brief and clear summary suitable for quick sharing on platforms like Slack or social media. Focus on capturing the essence and most impactful point of the text in one or two sentences.
...
export function buildPrompt(
  size: SummarySizeType,
  text: string,
): { system: string; prompt: string } {
  return {
    system: SYSTEM_PROMPTS[size],
    prompt: text,
  };
}
